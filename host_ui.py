...
class TestApplication(QWidget):
    ...
    def __init__(self, interface=None, back_callback=None, log=None):
    ...
    def initialize_host_ui(self):
        ...
        dump_logs_label.setObjectName("DumpLogs")
    ...
    def handle_pairing_request(self, request_type, device, uuid=None):
        ...
        elif request_type == "authorize_device":
            reply = QMessageBox.question(
                self, "Authorize Device Connection",
                f"Allow {device_address} to connect?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            if reply == QMessageBox.StandardButton.Yes:
                QMessageBox.information(self, "Connection Authorized", f"{device_address} is allowed to connect.")
                return True
            else:
                QMessageBox.information(self, "Connection Denied", f"{device_address} is not allowed to connect.")
                self.log.warning("User denied device connection authorization for %s", device_address)
                return False
